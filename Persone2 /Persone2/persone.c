...
struct Persona * eliminaPeso(struct Persona * lista, int num_peso){
    ...
    if(lista==NULL)
        return lista;
    if((lista->peso) / ((lista->altezza/100.00)*(lista->altezza/100.00)) > 25.00){
        prossima=lista->next;
        free(lista);
...
{
  while (lista!=NULL) {
      fprintf(fp,"%10.10s\t %10.10s\t %2d\t %3d\t %2d\t\n",lista->nome,lista->cognome,lista->peso,lista->altezza,lista->eta);
    lista = lista->next;
  }
}
...
struct Persona * ordinaLista(struct Persona * lista){
    ...
    while(curr!=NULL && curr->peso < lista->peso){  //ordino secondo il numero di viaggi dal più grande al più piccolo se voglio fare il contrario basta mettere al posto di ">" questo "<"
        prev=curr;
        curr=curr->next;
    }
